...
```python
ctrl.__init__(refreshRate=30, timeout=2) → The constructor for the controller. 
	Refreshrate denotes how often the controller refreshes, the timeout 
	refers to initial connection time limit.
...
.leftStick()       → returns a tuple (x, y) values scaled between -1.0 and 1.0
.rightStick()      → returns a tuple (x, y) values scaled between -1.0 and 1.0
```
